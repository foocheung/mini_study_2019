...
tm<- gmtPathways("./c2.cp.reactome.v7.1.symbols.gmt")
...
##+ facet_wrap(Ann~.,scales="free",strip.position="left")
...
allfgsea_all5<-rbind(allfgsea_all1,allfgsea_all2,allfgsea_all3,allfgsea_all4)



p1<-ggplot(allfgsea_all5, aes(y =reorder(pathway,-log10(padj)) , x = grp, size = -log10(padj), fill = NES)) + 
  geom_point(shape = 21) + 
...
pdf(paste("./miniReactome_",threshold,".pdf",sep=''),height=24,width=24)
...
write.table(as.matrix(allfgsea_all5),paste("./miniReactome_",threshold,".txt",sep=''),sep="\t")