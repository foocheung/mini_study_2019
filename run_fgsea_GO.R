...
tm<- gmtPathways("./c5.bp.v7.1.symbols.gmt")
...
p4<-ggplot(allfgsea_all5, aes(y =reorder(pathway,-log10(padj)) , x = grp, size = -log10(padj), fill = NES)) + 
  geom_point(shape = 21) + 
...
pdf(paste("./miniGO_",threshold,".pdf",sep=''),height=28,width=24)
p4
dev.off()


write.table(as.matrix(allfgsea_all5),paste("./miniGO_",threshold,".txt",sep=''),sep="\t")
